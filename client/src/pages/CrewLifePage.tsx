  ...
  return (
    <section className="py-12 border-b border-gray-200 last:border-b-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Featured article takes left 2/3 */}
...
export default function CrewLifePage() {
  const { data: crewLifeArticles = [], isLoading: loadingCrew } = useArticlesByCategory('crew-life', 20);
  const { data: captainsArticles = [], isLoading: loadingCaptains } = useArticlesByCategory('captains', 10);
  const { data: destinationsArticles = [], isLoading: loadingDest } = useArticlesByCategory('destinations', 10);

  const isLoading = loadingCrew || loadingCaptains || loadingDest;
      ...
      <main className="flex-1">
        ...
        <div className="border-b border-gray-200">
          <div className="container mx-auto px-6 md:px-8 py-4">
            <div className="flex items-center gap-2 text-sm text-gray-500">
              ...
              <span>›</span>
              <span className="text-gray-900 font-medium">Crew Life</span>
            </div>
          </div>
        </div>
        ...
        <div className="container mx-auto px-6 md:px-8 pt-8 pb-4">
          ...
          <p className="text-gray-600 mt-4 max-w-2xl">
            Stories, advice, and insights for the people who make the yachting world go round — from the bridge to the galley and everywhere in between.
          </p>
        </div>
